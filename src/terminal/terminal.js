...
function showCommandPrompt() {
    ...
    fend.DOM.commandContainer.scrollIntoView({
        behavior: "smooth",
        block: 'nearest',
        inline: 'nearest'
    });
...
function createCTFCfg() {
    const username = SECURITY_CONF.credential.username;
    ...
    return Object.freeze({
        ...
        username,
        password: SECURITY_CONF.credential.password,
        home,
        flag1_path: `${home}/flag.txt`,
        hint1_path: `${home}/notes.txt`
    });
}

const CTFCfg = createCTFCfg();

function init_ctf() {
    writeFile(CTFCfg.flag1_path, CTFCfg.flag1, 0o600);
    writeFile(CTFCfg.hint1_path, CTFCfg.hint1, 0o600);

    const pwnam = getpwnam(CTFCfg.username);
    chown(CTFCfg.flag1_path, pwnam.uid, pwnam.gid);
    chown(CTFCfg.hint1_path, pwnam.uid, pwnam.gid);

    writeFile(`/root/flag.txt`, CTFCfg.flag2, 0o600);
    writeFile("/etc/sudoers", CTFCfg.hint2, 0o644)
}

function init_system() {
    appendFile("/etc/passwd", `${CTFCfg.username}:x:1000:1000::${CTFCfg.home}:/bin/bash\n`);
    appendFile("/etc/group", `${CTFCfg.username}:x:1000:\n`);
    appendFile("/etc/shadow", `${CTFCfg.username}:${CTFCfg.password}:2048:0:99999:7:::\n`);
    appendFile("/etc/gshadow", `${CTFCfg.username}:*:1000:\n`);

    mkdir(`${CTFCfg.home}`, 0o750);
    const pwnam = getpwnam(CTFCfg.username);
    chown(`${CTFCfg.home}`, pwnam.uid, pwnam.gid);
}
